...
object VoronoiModel {
  ...
  private def chain( iPts: Map[Vector3, Set[Int]], acc: Seq[( Vector3, Set[Int] )] ): Seq[Vector3] = {
    if ( iPts.isEmpty )
      ...
      val ids = iPts.head._2
      val t = iPts.tail
      t.find( _._2.intersect( ids ).size == 1 ) match {
        case None =>         chain( Map.empty, acc )
        case Some( next ) => chain( t, next +: acc )
      }
    }
  }
  def chain( iPts: Map[Vector3, Set[Int]] ): Seq[Vector3] = if ( iPts.isEmpty ) Seq.empty else chain( iPts, Seq( iPts.head ) )

  def maybeFlip( n: Normal3, pts: Seq[Vector3] ): Seq[Vector3] = {
    if ( pts.length > 2 ) {
      val ptsNormal = ( pts( 1 ) - pts( 0 ) ) cross ( pts( 2 ) - pts( 1 ) )
      val k = ptsNormal dot n
      if ( k < 0 )
        pts.reverse
      else
        pts
